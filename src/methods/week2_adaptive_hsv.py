...
def week2_adaptive_hsv(video: Video, debug=False) -> Iterator[Frame]:
    ...
    for im, mask in gaussian_model_adaptive(video, int(2141 * 0.25), model_mean, model_std, total_frames=int(2141 * 0.10),
                                        pixel_value=PixelValue.HSV, alpha=1.75, rho=0.01):
        mask = mask & roi
        if debug:
            cv2.imshow('f', mask)
            cv2.waitKey()
